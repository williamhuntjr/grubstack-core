...
export const RestaurantColors: FC<IRestaurantColors> = ({ onUpdate, state }) => {
  ...
  const handleOpenDialog = (property: RestaurantProperty): void => {
    setColor(filterProperty(state.properties, property) ?? '')
    ...
    <div className={styles.restaurantColors}>
      ...
      >
        <div>
          ...
          />
          <Button variant="contained" color="primary" onClick={() => onUpdate(colorPickerDialogData, color)}>Save</Button>
        </div>
      </GrubDialog>
      <h2>Colors</h2>
      <Divider />
      <div className={styles.colorsList}>
        ...
        <div className={styles.colorContainer} key={index}>
          ...
          <div className={styles.colorPreview} style={{backgroundColor: filterProperty(state.properties, restaurantColor.property) ?? ''}} />
          <Button variant="contained" color="primary" onClick={() => handleOpenDialog(restaurantColor.property)}>Change Color</Button>
        </div>
        )}
      </div>
    </div>
