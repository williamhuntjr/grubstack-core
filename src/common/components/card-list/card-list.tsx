      ...
      <div className={styles.cardListContainer}>
        {data.map((item, index) =>
          <div key={index} className={cls(
            ...
            <div className={cls(styles.cardListItemContent, layout == "vertical" ? styles.cardListItemContentVertical : '', onClick ? styles.cardListClickable : '')} onClick={onClick ? () => onClick(item) : () => {}} onKeyDown={onClick ? () => onClick(item) : () => {}} role="button" tabIndex={index}>
            {layout !== "vertical" &&
              <div className={styles.cardListImage}><img src={item.thumbnail_url} alt="" /></div>
            }
                ...
                <div className={cls(styles.buttonContainer, layout == "vertical" ? styles.buttonContainerVertical : '')}>
                  ...
                  {actions.map((action, sindex) => 
                    <Tooltip title={action.label} key={sindex}>
                      <Button variant={buttonStyle} color={buttonColor} aria-label={action.label} className={styles.cardButton} onClick={() => {
