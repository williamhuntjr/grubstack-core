...
export function usePagination<TData, TFilters = {}>(
  requestFn: TPaginationRequestFn<TData, TFilters>,
  pageLimit?: number,
  filters?: TFilters | undefined
): IPaginationHook<TData, TFilters> {
  const [state, setState] = useState<IPaginationHookState<TData, TFilters>>({
    isLoading: true,
    data: [],
    total: 0,
    pages: 1,
    pagination: { page: 1, limit: pageLimit ?? listPageSize },
    filters: filters,
  })

  const fetchData = useCallback(
    async (pagination?: IPaginationParams, newFilters?: TFilters): Promise<void> => {
