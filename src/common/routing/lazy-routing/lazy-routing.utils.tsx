...
export function buildAsyncRoute<T extends IModuleDefinition = never>(
  route: IAsyncRoute<T>,
  render: (route: IRoute) => React.ReactElement
): JSX.Element {
  return <Route key={route.path} path={route.path} element={<LazyRoutingModuleLoader route={route} render={render} key={route.path} />} />
}
